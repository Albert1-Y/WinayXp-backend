...
const MostrarActividad = async (req, res) => {
  try {
    ...
    const existe = await AdminModel.mostrarActividad({
      fecha_inicio,
      fecha_fin,
    });
...
const actualizarAsistenciaEstudiante = async (req, res) => {
  try {
    ...
    if (!estudianteId && id_persona) {
      const estudiante = await AdminModel.obtenerEstudiantePorIdPersona({
        id_persona,
      });
      if (!estudiante) {
    ...
    const { asistencia, totales, nivel, nivelesPendientes } =
      await AdminModel.guardarAsistenciaEstudiante({
